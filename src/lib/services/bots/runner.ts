...
async function processSourcelessForecast(
  ...
  try {
    ...
    if (bot.requireApprovalForForecasts) {
      prediction = await prisma.prediction.create({ data: predictionCreateData })
      await prisma.prediction.update({ where: { id: prediction.id }, data: { status: publishStatus, publishedAt: new Date() } })
    } else {
      const stake = randomInt(bot.stakeMin, bot.stakeMax)
      stakeAmount = stake
      try {
        const out = await prisma.$transaction(async (tx) => {
          const pred = await tx.prediction.create({ data: predictionCreateData })
          await tx.prediction.update({ where: { id: pred.id }, data: { status: publishStatus, publishedAt: new Date() } })
        ...
        prediction = out.prediction
        emitCreateCommitmentSideEffects({ ...out.prediction, options: [] } as CommitmentPrediction, out.stakeResult.data, { confidence: stake })
      } catch (stakeErr) {
