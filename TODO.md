...
- [ ] **Feature: Private (unlisted) forecasts** — allow forecast authors to mark a forecast as private/unlisted: visible only to the author and users with a direct link. Requires: `isPublic` boolean on `Prediction` model (default `true`), API-level filtering (`where: { OR: [{ isPublic: true }, { authorId: session.user.id }] }`), UI toggle on the create/edit forecast screen, and hiding private forecasts from the main feed, search, and leaderboard aggregations.
