...
- [ ] **Next.js 15** — already on 15.5.12; track for stable LTS and React 19 compatibility. ✅ *Already running 15.5.x — monitor for breaking changes in minor updates.*
...
- [x] Upgrade Next.js to ≥14.2.35 (CVE GHSA-f82v-jwr5-mffw middleware auth bypass)
...
- [x] Bedrock Phase 1 — Terraform IAM + SSM parameters
- [x] Bedrock Phase 1 — Create 4 prompts in Bedrock console
- [x] Bedrock Phase 1 — Populate SSM params with Version 1 ARNs
- [x] Bedrock Phase 1 — App integration (bedrock-prompts.ts, 5-min TTL cache)
- [x] Bedrock Phase 1 — promote-prompt.sh with rollback support
- [x] Bedrock Phase 2 — migrate all 9 remaining inline prompts to Bedrock
...
- [x] Notifications: MENTION type wired in comments POST
- [x] Notifications: silent UI errors → error toasts in NotificationPreferences + NotificationList
- [x] Notifications: dispatchBrowserPush retry + batch DB updates
...
- [x] Security: fix unlisted forecasts leaking into ?resolvedOnly and ?status= queries
...
- [x] Security: rate-limit context updates per user (10/day across all forecasts)
- [x] Security: env var validation at startup (instrumentation.ts)
...
- [x] Testing: coverage expansion (759 tests across 59 files)
- [x] Testing: E2E smoke tests (Playwright — home, sign-in, 404, /api/health)
- [x] About page (version, commit SHA, build date)
- [x] Docs: VAPID setup in SECRETS.md
- [x] Fix: resolve double-submit (success banner replaces form immediately)
